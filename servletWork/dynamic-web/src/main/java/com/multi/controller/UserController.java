...
/*
controller가 있는 이유?(Model과 View가 분리하므로 인해서 생겨났다.)
controller는 Mapper역할, 해당 Action을 불러주면 로직을 수행하고
redirect 또는 forward방식으로 결과페이지, 또다른 Action을 부른다.
 */
@WebServlet("/user")
public class UserController extends HttpServlet { //UserController는 UserService를 가지고 있다.
    private UserService userService = new UserServiceImpl(); //의존객체
