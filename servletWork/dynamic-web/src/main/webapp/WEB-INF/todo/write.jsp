...
<div class="wrap">
    <h2>새 할 일</h2>
    <form method="post" action="${pageContext.request.contextPath}/todo/write">
        <div class="field">
            <label>제목 *</label>
        ...
        <div class="field">
            <label>내용</label>
            <textarea name="description" rows="6">${form.description}</textarea>
            <c:if test="${not empty errors.description}"><div class="err">${errors.description}</div></c:if>
        </div>

        <div class="field">
            <label>카테고리</label>
            <input type="text" name="category" maxlength="40" value="${form.category}"/>
        ...
        <div class="field">
            ...
            <select name="priority">
                <c:forEach var="i" begin="1" end="5">
                    <option value="${i}" ${form.priority==i?'selected':''}>${i}</option>
                </c:forEach>
            </select>
        ...
        <div class="field">
            <label>상태 *</label>
            <select name="status">
                <c:set var="st" value="${empty form.status ? 'TODO' : form.status}"/>
        ...
        <div class="field">
            <label>마감일</label>
            <input type="datetime-local" name="dueAt" value="${form.dueAt}"/>
            <c:if test="${not empty errors.dueAt}"><div class="err">${errors.dueAt}</div></c:if>
        </div>

        <div class="actions">
            <button type="submit" class="btn primary">저장</button>
            <a class="btn" href="${pageContext.request.contextPath}/todo/list">취소</a>
        </div>
    </form>
</div>
</body>
